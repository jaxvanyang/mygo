...
class Move:
    ...
    def from_sgf(cls, sgf_coords, board_size, player="B"):
        ...
        if (
            sgf_coords == "" or Move.SGF_COORD.index(sgf_coords[0]) == board_size[0]
        ):  # some servers use [tt] for pass
            return cls(coords=None, player=player)
        return cls(
            coords=(
            ...
            if isinstance(item, str):
                ...
                if len(item.children) == 1:
                    ...
                    stack += sum(
                        [
                            [")", c, "("]
                            for c in self.order_children(item.children)[::-1]
                        ],
                        [],
                    )
        return sgf_str
